"""
Visualize Failed GOT-10k Sequences with Phase 2 Improved Tracker

This script re-runs and visualizes GOT-10k sequences that previously failed
with the baseline tracker, using the improved HiM2SAM-style tracker.
"""
...
def get_failed_sequences() -> List[str]:
    ...
    for f in sorted(FAILED_VIDEO_DIR.glob("*.mp4")):
        # Extract sequence name from filename like "GOT-10k_Val_000020_failed.mp4"
...
def create_tracking_video(
    ...
    for i in range(num_frames):
        ...
        # Draw predicted bbox (red for visible, yellow for occluded)
        if pred_bbox:
            color = (0, 0, 255)  # Red for visible
            vis_frame = draw_bbox(vis_frame, pred_bbox, color, "Pred", 2)
        ...
        for j, text in enumerate(info_text):
            color = (255, 255, 255)
            if "OCCLUDED" in text:
                color = (0, 255, 255)  # Yellow for occluded
            cv2.putText(vis_frame, text, (10, 25 + j * 22), font, 0.55, color, 2)

        # Legend
        legend_y = height - 80
        cv2.rectangle(vis_frame, (10, legend_y), (250, height - 10), (0, 0, 0), -1)
...
def process_sequence(
    ...
    # Track
    print(f"  Running Phase 2 Improved Tracking ({num_frames} frames)...")
    ...
    # Create video
    output_path = output_dir / f"{sequence_name}_phase2improved.mp4"
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize Failed GOT-10k Sequences with Phase 2 Improved Tracker",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument("--sequence", type=str, default=None,
                        help="Specific sequence to process (e.g., GOT-10k_Val_000020)")
    parser.add_argument("--all", action="store_true",
                        help="Process all failed sequences")
    parser.add_argument("--max-sequences", type=int, default=5,
                        help="Maximum sequences to process")
    parser.add_argument("--max-frames", type=int, default=0,
                        help="Maximum frames per sequence (0=all)")
    parser.add_argument("--chunk-size", type=int, default=200,
                        help="Chunk size for SAM2 processing")
    parser.add_argument("--list", action="store_true",
                        help="List all failed sequences and exit")
    ...
    print("="*70)
    print("PHASE 2 IMPROVED - FAILED GOT-10K VISUALIZATION")
    print("="*70)
    ...
    output_dir = Path(args.output_dir) if args.output_dir else VIDEO_OUTPUT_DIR / "failed_got10k_phase2improved"
    output_dir.mkdir(parents=True, exist_ok=True)

    # Select sequences to process
    if args.sequence:
    ...
    if results:
        ...
        summary_path = RESULTS_DIR / "failed_got10k_phase2improved_results.json"
