"""
Baseline Video Visualization Script

This script creates video visualizations of baseline tracking results
using SAM 2 model (no Kalman filter).
"""
...
# ============================================================================
# Dataset Loaders (from old visualize_baseline_video.py)
# ============================================================================
...
def create_tracking_video(img_dir: str, gt_bboxes: List[List[int]], pred_bboxes: List[List[int]],
    ...
    for i in range(num_frames):
        ...
        legend_y = height - 60
        cv2.rectangle(vis_frame, (10, legend_y), (220, height - 10), (0, 0, 0), -1)
        ...
        cv2.putText(vis_frame, "Baseline", (50, legend_y + 42), font, 0.5, (255, 255, 255), 1)
...
# Main Processing
# ============================================================================
def process_sequence(dataset: str, sequence_name: str, tracker: SAM2Tracker,
    ...
    # Track
    print(f"  Running Baseline Tracking...")
    num_frames = min(max_frames, num_frames) if max_frames > 0 else num_frames
    pred_bboxes, _ = tracker.track(img_dir, gt_bboxes[0], num_frames, chunk_size=chunk_size)

    # Render
    output_path = VIDEO_OUTPUT_DIR / f"{dataset}_{sequence_name}_baseline.mp4"
...
def main():
    parser = argparse.ArgumentParser(description="Baseline Tracker Video Visualization")
    ...
    tracker = SAM2Tracker(sam2_config=CONFIG["sam2_config"], sam2_checkpoint=str(CONFIG["sam2_checkpoint"]))
    ...
    if results:
        ...
        summary_path = RESULTS_DIR / f"{args.dataset}_baseline_video_results.json"
        with open(summary_path, 'w') as f:
            json.dump({"dataset": args.dataset, "model": "Baseline SAM 2", "sequences": results, "overall_avg_iou": avg_iou}, f, indent=2)
